...
public class Q3009 {
    public static void main(String[] args) throws IOException {
        ...
        if (arRect[0][0] == arRect[1][0]) {
            IO.write(Cast.ItoString(arRect[2][0]) + " ");
        } else if (arRect[1][0] == arRect[2][0]) {
            IO.write(Cast.ItoString(arRect[0][0]) + " ");
        } else if (arRect[0][0] == arRect[2][0]) {
            IO.write(Cast.ItoString(arRect[1][0]) + " ");
        }

        if (arRect[0][1] == arRect[1][1]) {
            IO.write(Cast.ItoString(arRect[2][1]));
        } else if (arRect[1][1] == arRect[2][1]) {
            IO.write(Cast.ItoString(arRect[0][0]));
        } else if (arRect[0][1] == arRect[2][1]) {
            IO.write(Cast.ItoString(arRect[1][1]));
        }
